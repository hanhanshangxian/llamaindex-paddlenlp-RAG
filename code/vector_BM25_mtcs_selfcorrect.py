...
# # 一、加载本地lora微调后模型
# # 1. 设置模型路径
# model_path = "/home/wwh/PaddleNLP/llm/checkpoints/lora_ckpts2"
...
def chat_loop_with_query_engine():
    ...
    while True:
        ...
        try:
            ...
            new_queries = [f"{user_input} {result}" for result in nodes]
            extended_results = []
            for new_query in new_queries:
                results = retriever._retrieve(QueryBundle(query_str=new_query))
            ...
            scored_results = []
            for node in extended_results:
                relevance_score = calculate_relevance_score(node.text, user_input)
            ...
            top_k = 3
            selected_chunk_combinations = select_best_chunks_with_mcts(
                [result[0] for result in sorted_results],  # 提取节点用于 MCTS
                query=user_input,
                budget=8192,
                top_k=top_k
            )
...
def calculate_relevance_score(text: str, query: str) -> float:
    ...
    if text_embedding.size == 0 or query_embedding.size == 0:
        return 0.0
    similarity = cosine_similarity(text_embedding, query_embedding).flatten()[0]
    return similarity
